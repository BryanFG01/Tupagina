...
export function NavbarBlock({ content, landingId = '', currency = 'usd', previewMode = false }: Props) {
  ...
  const bgCls     = content.transparent && !scrolled
    ? 'bg-transparent'
    : `bg-[${content.backgroundColor}]`

  // Custom bg style because Tailwind JIT won't know the hex value
  const bgStyle: React.CSSProperties = {
    backgroundColor:
      content.transparent && !scrolled ? 'transparent' : content.backgroundColor,
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            {content.ctaText && (
              <a
                href={content.ctaUrl ?? '#'}
                className="hidden md:inline-flex items-center px-4 py-1.5 text-sm font-semibold rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors shadow-sm"
              >
                {content.ctaText}
              </a>
            )}
              ...
              <>
                ...
                >
                  ...
                  {(count > 0 || previewMode) && (
                    ...
                    >
                      {previewMode && count === 0 ? '3' : count > 9 ? '9+' : count}
                    </span>
                  )}
                </button>
              </>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden border-t" style={{ borderColor: isDark ? '#e5e7eb' : 'rgba(255,255,255,0.2)', backgroundColor: content.backgroundColor }}>
          <div className="px-4 py-3 space-y-1">
            ...
            {content.ctaText && (
              <div className="pt-2">
                <a
                  href={content.ctaUrl ?? '#'}
                  className="block w-full text-center px-4 py-2 text-sm font-semibold rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors"
                  onClick={() => setMobileOpen(false)}
                >
                  {content.ctaText}
                </a>
