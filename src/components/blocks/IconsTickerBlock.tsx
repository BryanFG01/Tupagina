...
export function IconsTickerBlock({ content }: Props) {
  ...
  // Genera los puntos SVG de un starburst con N puntas
  function starburstPoints(cx: number, cy: number, outerR: number, innerR: number, numSpikes: number): string {
    const pts: string[] = []
  ...
  function BadgeEl({ badge }: { badge: IconTickerBadge }) {
    ...
    if (badge.shape === 'starburst') {
      const pts = starburstPoints(50, 50, 48, 34, 10) // 10-spike burst
      return (
