...
const DEMO_PRODUCTS: Product[] = [
  { id: 'd1', landingId: 'demo', name: 'Camiseta Clásica',   category: 'Ropa',        description: 'Algodón 100%, corte regular. Disponible en varios colores.',               imageUrl: null, price: 2900,  comparePrice: null, stock: -1, badge: 'Nuevo',     active: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'd2', landingId: 'demo', name: 'Pantalón Cargo',     category: 'Ropa',        description: 'Diseño moderno con múltiples bolsillos. Comodidad todo el día.',            imageUrl: null, price: 4900,  comparePrice: 6900, stock:  8, badge: 'Oferta',    active: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'd3', landingId: 'demo', name: 'Zapatillas Urban',   category: 'Calzado',     description: 'Suela de goma antideslizante. Estilo urbano para cualquier ocasión.',       imageUrl: null, price: 9900,  comparePrice: null, stock:  3, badge: 'Popular',   active: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'd4', landingId: 'demo', name: 'Gorra Streetwear',   category: 'Accesorios',  description: 'Cap ajustable. Diseño exclusivo con bordado en relieve.',                   imageUrl: null, price: 1900,  comparePrice: null, stock: -1, badge: null,        active: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'd5', landingId: 'demo', name: 'Hoodie Oversized',   category: 'Ropa',        description: 'Sudadera con capucha. Tela pesada premium. Fit oversized.',                 imageUrl: null, price: 6900,  comparePrice: 8500, stock: 12, badge: 'Oferta',    active: true, createdAt: new Date(), updatedAt: new Date() },
  { id: 'd6', landingId: 'demo', name: 'Bolso Tote',         category: 'Accesorios',  description: 'Lona resistente, asas reforzadas. Ideal para el día a día.',                imageUrl: null, price: 3500,  comparePrice: null, stock:  0, badge: null,        active: true, createdAt: new Date(), updatedAt: new Date() },
]
...
const BADGE_COLORS: Record<string, string> = {
  'Nuevo':     'bg-indigo-600 text-white',
  'Oferta':    'bg-red-500    text-white',
  'Popular':   'bg-amber-500  text-white',
  'Destacado': 'bg-emerald-500 text-white',
}
...
function Badge({ text }: { text: string }) {
  const cls = BADGE_COLORS[text] ?? 'bg-gray-700 text-white'
  return (
    <span className={`absolute top-2.5 left-2.5 text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider shadow-sm ${cls}`}>
      {text}
    </span>
  )
}
...
function ProductCard({ product, buttonText, formatted, onAdd, onRemoveOne, onViewDetail, previewMode, cartQty }: {
  product: Product
  buttonText: string
...
function ProductListRow({ product, buttonText, formatted, onAdd, onRemoveOne, onViewDetail, previewMode, cartQty }: {
  product: Product
  buttonText: string
  formatted: (cents: number) => string
  onAdd: (() => void) | null
  ...
  return (
    <div className="flex items-center gap-4 bg-white border border-gray-100 rounded-2xl p-4 hover:shadow-md hover:border-gray-200 hover:-translate-y-px transition-all duration-200">
      ...
      <div className="relative w-20 h-20 flex-shrink-0 rounded-xl bg-gray-50 overflow-hidden">
        ...
        {product.badge && (
          <span className={`absolute top-1 left-1 text-[9px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wide ${BADGE_COLORS[product.badge] ?? 'bg-gray-700 text-white'}`}>
            {product.badge}
          </span>
        )}
        {outOfStock && (
