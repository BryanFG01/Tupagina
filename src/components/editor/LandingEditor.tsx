...
import type { Block, BlockType } from '@/domain/landing/block.types'
import { BLOCK_DEFAULTS } from '@/domain/landing/block.types'
import { updateLandingAction, publishLandingAction } from '@/app/actions/landing.actions'
...
const IconIconTicker = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
    ...
    <line x1="3" y1="14" x2="7" y2="14"/><line x1="10.5" y1="14" x2="13.5" y2="14"/><line x1="18" y1="14" x2="21" y2="14"/>
  </svg>
)
const IconMarcas = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
...
const BLOCK_META: Record<BlockType, BlockMeta> = {
  hero:               { label: 'Portada',           icon: <IconPortada />,          desc: 'Título + CTA',             color: 'bg-violet-50 text-violet-600 border-violet-200' },
  services:           { label: 'Servicios',          icon: <IconServicios />,        desc: 'Grid de servicios',        color: 'bg-amber-50  text-amber-600  border-amber-200'  },
  testimonials:       { label: 'Testimonios',        icon: <IconTestimonios />,      desc: 'Reseñas de clientes',      color: 'bg-sky-50    text-sky-600    border-sky-200'    },
  payment:            { label: 'Botón de pago',      icon: <IconPago />,             desc: 'Checkout integrado',       color: 'bg-green-50  text-green-600  border-green-200'  },
  contact:            { label: 'Contacto',           icon: <IconContacto />,         desc: 'WhatsApp y email',         color: 'bg-rose-50   text-rose-600   border-rose-200'   },
  store:              { label: 'Tienda',             icon: <IconTienda />,           desc: 'Catálogo + carrito',       color: 'bg-indigo-50 text-indigo-600 border-indigo-200' },
  'store-banner':     { label: 'Banner tienda',      icon: <IconBannerTienda />,     desc: 'Hero + botón carrito',     color: 'bg-fuchsia-50 text-fuchsia-600 border-fuchsia-200' },
  ...
  features:           { label: 'Features/Beneficios', icon: <IconFeatures />,       desc: 'Cards con imagen o ícono', color: 'bg-blue-50   text-blue-600   border-blue-200'    },
  'icons-ticker':     { label: 'Banner de íconos',   icon: <IconIconTicker />,      desc: 'Íconos animados + links',  color: 'bg-yellow-50 text-yellow-600 border-yellow-200' },
  'loading-spinner':  { label: 'Spinner de carga',   icon: <IconSpinner />,         desc: 'Pantalla de carga custom', color: 'bg-purple-50 text-purple-600 border-purple-200' },
}
...
export function LandingEditor({ landing }: Props) {
  ...
  function addBlock(type: BlockType) {
    ...
    const block = { id: nanoid(), type, order: blocks.length, content: JSON.parse(JSON.stringify(BLOCK_DEFAULTS[type])) } as Block
  ...
  async function handlePublish() {
    ...
    setPublishing(false)
    if (result.success) { setIsPublished(result.data.published); router.refresh() }
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F0F2F5] overflow-hidden font-sans">
      ...
      <header className="flex items-center justify-between px-5 h-14 bg-white border-b border-gray-200/80 shadow-sm z-30 flex-shrink-0">
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-800 truncate leading-tight">{landing.title}</p>
            <p className="text-[11px] text-gray-400 leading-tight">tunegocio.app/p/{landing.slug}</p>
          </div>
          {isPublished && (
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {saveMsg && (
            <span className={`text-xs font-semibold hidden sm:block px-2.5 py-1 rounded-lg ${
              saveMsg.includes('✓')
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* ── LEFT SIDEBAR ── */}
        <aside className="w-52 bg-white border-r border-gray-200/80 flex flex-col flex-shrink-0 shadow-sm">
          ...
          <div className="p-4 border-t border-gray-100 space-y-2">
            <button
              onClick={handlePublish}
              disabled={publishing}
              className={`w-full py-2.5 rounded-xl font-semibold text-sm transition-all shadow-sm disabled:opacity-60 ${
                isPublished
                  ? 'bg-emerald-500 hover:bg-emerald-600 text-white'
                  : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-200'
              }`}
            >
              {publishing ? '…' : isPublished ? '✓ Publicada' : 'Publicar página'}
            </button>
            <p className="text-center text-[11px] text-gray-400">
              {blocks.length} bloque{blocks.length !== 1 ? 's' : ''} en tu página
            </p>
          </div>
        </aside>
              ...
              <DragOverlay dropAnimation={{ duration: 200, easing: 'cubic-bezier(0.18,0.67,0.6,1.22)' }}>
                {dragActiveBlock && (
                  <div className="bg-white rounded-2xl shadow-2xl shadow-black/20 border border-indigo-300 px-4 py-3 flex items-center gap-3 rotate-1 scale-105">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-800">{BLOCK_META[dragActiveBlock.type].label}</p>
          ...
          {selectedBlock && (
            <>
              ...
              <div className="px-4 py-4 border-b border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <span className={`w-9 h-9 rounded-xl border flex items-center justify-center text-base flex-shrink-0 ${BLOCK_META[selectedBlock.type].color}`}>
                      {BLOCK_META[selectedBlock.type].icon}
                    </span>
                    <div>
                      <p className="text-sm font-bold text-gray-800 leading-tight">{BLOCK_META[selectedBlock.type].label}</p>
                      <p className="text-[11px] text-gray-400 font-mono">#{selectedBlock.id.slice(0, 8)}</p>
                    </div>
                  </div>
                  <button
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {rightTab === 'content' ? (
                  <>
                    {selectedBlock.type === 'hero'         && <HeroBlockEditor         content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'services'     && <ServicesBlockEditor     content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'testimonials' && <TestimonialsBlockEditor content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'payment'      && <PaymentBlockEditor      content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'contact'      && <ContactBlockEditor      content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'store'             && <StoreBlockEditor             content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'store-banner'      && <StoreBannerBlockEditor       content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'floating-buttons'  && <FloatingButtonsBlockEditor   content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'footer'            && <FooterBlockEditor            content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'faq'              && <FaqBlockEditor               content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'navbar'           && <NavbarBlockEditor            content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'brands-banner'   && <BrandsBannerBlockEditor      content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'gallery'         && <GalleryBlockEditor            content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'stats'           && <StatsBlockEditor              content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'features'        && <FeaturesBlockEditor           content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'icons-ticker'    && <IconsTickerBlockEditor        content={selectedBlock.content} onChange={updateSelected} />}
                    {selectedBlock.type === 'loading-spinner' && <SpinnerBlockEditor             content={selectedBlock.content} onChange={updateSelected} />}
                  </>
                ) : (
                  <BlockStylePanel
              ...
              <div className="p-4 border-t border-gray-100 space-y-2">
                <button
                  onClick={async () => { setSelectedId(null); await handleSave() }}
                  disabled={saving}
...
function SortableBlock({ block, selected, isDragging, meta, onClick, onRemove, onDuplicate }: SortableBlockProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Top-left label pill */}
        <div className={`absolute top-2 left-3 flex items-center gap-1.5 transition-opacity duration-150 ${
          selected ? 'opacity-100' : 'opacity-0 group-hover/block:opacity-100'
        }`}>
          <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full shadow-md border ${meta.color} bg-white`}>
        ...
        {/* Top-right actions */}
        <div className={`absolute top-2 right-3 flex items-center gap-1 transition-opacity duration-150 ${
          selected ? 'opacity-100' : 'opacity-0 group-hover/block:opacity-100'
        }`}>
          {/* Drag handle */}
          <div
...
function ArrowLeftIcon() {
  return <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
}

function SaveIcon() {
  return <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z"/><polyline points="17,21 17,13 7,13 7,21"/><polyline points="7,3 7,8 15,8"/></svg>
}

function PublishIcon() {
  return <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
}

function ExternalLinkIcon() {
  return <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15,3 21,3 21,9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
}
...
function TrashIcon() {
  return <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3,6 5,6 21,6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
}

function SpinnerIcon() {
  return <svg className="w-3.5 h-3.5 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
}
...
function DesktopIcon() {
  return <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
}

function MobileIcon() {
  return <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="5" y="2" width="14" height="20" rx="2"/><circle cx="12" cy="18" r="1" fill="currentColor"/></svg>
}
