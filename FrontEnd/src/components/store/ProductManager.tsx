...
import type { StoreBadgePreset } from '@/domain/landing/block.types'
import {
  createProductAction,
  updateProductAction,
  deleteProductAction,
...
type Props = { landingId: string; initialProducts: Product[]; badgePresets?: StoreBadgePreset[] }
...
const EMPTY_FORM: FormState = {
  ...
  category: '', badge: '', badgeShape: 'pill', badgeSize: 'md',
}

const DEFAULT_BADGE_PRESETS: StoreBadgePreset[] = [
  ...
  { id: 'bp-5', text: 'HOT 🔥',   bg: '#f97316', color: '#ffffff' },
  { id: 'bp-6', text: '2×1',      bg: '#ec4899', color: '#ffffff' },
  { id: 'bp-7', text: '-20%',     bg: '#dc2626', color: '#ffffff' },
  { id: 'bp-8', text: 'GRATIS',   bg: '#8b5cf6', color: '#ffffff' },
]
const STOCK_OPTIONS = [
...
export function ProductManager({ landingId, initialProducts, badgePresets: externalPresets }: Props) {
  const badgePresets = externalPresets ?? DEFAULT_BADGE_PRESETS
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 shadow-sm space-y-5">
          ...
          <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-gray-800">🏷️ Etiqueta del producto</p>
                <p className="text-[11px] text-gray-400 mt-0.5">Aparece sobre la imagen del producto en la tienda</p>
              </div>
              {form.badge && (
                <button
                  type="button"
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                      active
                        ...
                        : 'opacity-75 hover:opacity-100 hover:scale-105 hover:shadow-sm'
                    }`}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="O escribe tu propia etiqueta: Liquidación, -30%, SALE…"
                maxLength={20}
              ...
              />
              {form.badge && (() => {
                const preset = badgePresets.find(p => p.text === form.badge)
              ...
              <div className="grid grid-cols-2 gap-3 pt-1 border-t border-gray-200">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-1.5">
                    {([
                      { value: 'pill',       label: 'Píldora',  preview: '⬭' },
                      { value: 'square',     label: 'Cuadrada', preview: '▪' },
                      { value: 'starburst',  label: 'Estrella', preview: '✦' },
                    ] as const).map(opt => (
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() => setForm(f => ({ ...f, badgeShape: opt.value }))}
                        title={opt.label}
                        className={`flex-1 flex flex-col items-center gap-0.5 py-1.5 rounded-lg border text-[10px] font-semibold transition-all ${
                          form.badgeShape === opt.value
                  ...
                  <div className="flex gap-1.5">
                    {([
                      ...
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() => setForm(f => ({ ...f, badgeSize: opt.value }))}
                        className={`flex-1 py-1.5 rounded-lg border text-xs font-bold transition-all ${
                          form.badgeSize === opt.value
                            ? 'border-indigo-500 bg-indigo-50 text-indigo-700'
                            : 'border-gray-200 text-gray-500 hover:border-gray-300 hover:bg-gray-50'
                        }`}
                      >
