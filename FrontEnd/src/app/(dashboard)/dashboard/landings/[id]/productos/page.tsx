...
export default async function ProductosPage({ params }: Props) {
  ...
  // Extract badge presets from the store block (if any)
  const storeBlock = landing.blocks?.find((b: { type: string }) => b.type === 'store')
  const badgePresets = (storeBlock?.content as { badgePresets?: unknown })?.badgePresets as import('@/domain/landing/block.types').StoreBadgePreset[] | undefined
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <ProductManager landingId={id} initialProducts={products} badgePresets={badgePresets} />
    </div>
  )
}
